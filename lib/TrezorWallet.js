...
class TrezorWallet {
  constructor(networkId, accountsOffset = 0, accountsQuantity = 6, eventEmitter) {
    ...
    this.device = null;
    this.session = null;
/*    _trezorConnect.manifest({
      email: 'eugene.rupakov@gmail.com',
      appUrl: 'https://osadc.io'
    }); */
  }
  ...
  async signTransactionAsync(txData) {
    ...
    const txDataFormatted = Object.assign(null, txData);
    txDataFormatted.gasLimit = new _bignumber.default(txDataFormatted.gasLimit).toString(16);
    const txDataClone = Object.assign(null, txDataFormatted);
    ...
    const signedTx = new _ethereumjsTx.default(Object.assign( null, 
  ...
  async getAccounts(callback) {
    try {
      let r = { device: '', session: '' };
      if (this.session == null) {
        r = await deviceList.acquireFirstDevice(true);
      ...
      let result = await this.session.typedCall('GetPublicKey', 'PublicKey', addressN);
  ...
  async getPublicKey(address) {
    ...
    let result = await this.session.typedCall('GetPublicKey', 'PublicKey', {
  ...
  async signPersonalMessage(address, message) {
    ...
    return this.session.signEthMessage(this._getAddressByIndex(accountIndex), message);
  }
