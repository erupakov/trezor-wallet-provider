...
class TrezorWallet {
  ...
  async signTransactionAsync(txData) {
    ...
    let signPromise = session.signEthTx(_getAddressByIndex(accountIndex), txData.nonce, txData.gasPrice, txData.gasLimit, txData.to, txData.value, txData.data, chainId);
    let signed = null;
