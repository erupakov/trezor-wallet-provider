...
class EthUtils {
  ...
  static validateEtherAddress(address) {
    if (address.substring(0, 2) !== '0x') return false;else if (!/^(0x)?[0-9a-f]{40}$/i.test(address)) return false;else if (/^(0x)?[0-9a-f]{40}$/.test(address) || /^(0x)?[0-9A-F]{40}$/.test(address)) return true;else return EthUtils.isChecksumAddress(address);
  }
  ...
  static decimalToHex(dec) {
    return new _bignumber.BigNumber(dec).toString(16);
  }
  ...
  static hexToDecimal(hex) {
    if (hex === '0x') return 0;
    return new _bignumber.BigNumber(EthUtils.sanitizeHex(hex)).toString();
  }
  ...
  static getDeteministicContractAddress(address, nonce) {
    nonce = new _bignumber.BigNumber(nonce).toString();
  ...
  static getFunctionSignature(name) {
    return this.sha3(name).toString('hex').slice(0, 8);
  }

  static getBalanceDecimal(balance, decimal) {
    return new _bignumber.BigNumber(balance).div(new _bignumber.BigNumber(10).pow(decimal)).toString();
  }

}

var _default = EthUtils;
